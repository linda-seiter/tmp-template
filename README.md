...
NOTE: The `tree` command should already be installed on Windows and Linux
operating systems. You can install the command on a Mac with
`brew install tree`.

In the later step, you will add a `client` folder to contain your React
front-end application.

> **Note: You may already know some or all of the material covered in this
> lesson. We hope that having it all in one place will help you in designing and
> developing your project, regardless of where you're starting off.**
...
Run the following command to create the React front-end application in a folder
named `client`:
...
Change into the `client` directory to install the dependencies listed in
`client/package.json`. You will also need to install `react-router-dom`:
...
Edit `package.json` to add a `proxy` field:
...
Type `npm start` to confirm your React app launches in the browser.

Press `ctrl-C` to terminate the React app, then change directory out of the
`client` folder back to the project root folder:
...
being said, we haven't put much in there- just Python version 3.8.13 and ipdb.
...
Navigate to `models.py` and start creating your models. Remember to regularly
run `flask db revision --autogenerate -m'<descriptive message>'` and
...
You've already made a few, but let's take a look at the structure of your newest
